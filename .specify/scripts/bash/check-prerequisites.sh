...
eval $(get_feature_paths "$FEATURE_DIR_ARG") || exit 1
    ...
    if $PATHS_ONLY; then
        if $JSON_MODE; then
            printf '{"REPO_ROOT":"%s","FEATURE_DIR":"%s","FEATURE_DOCS_DIR":"%s","FEATURE_SPEC":"%s","CODE_DOC":"%s","RESEARCH":"%s","DATA_MODEL":"%s","QUICKSTART":"%s","SCREEN_ABSTRACTION":"%s","CONTRACTS_DIR":"%s","NAMING_SOURCE_KIND":"%s","NAMING_SOURCE_FILE":"%s","NAMING_SOURCE_REASON":"%s","NAMING_POLICY_JSON":%s}\n' \
            "$REPO_ROOT" "$FEATURE_DIR" "$FEATURE_DOCS_DIR" "$FEATURE_SPEC" "$CODE_DOC" "$RESEARCH" "$DATA_MODEL" "$QUICKSTART" "$SCREEN_ABSTRACTION" "$CONTRACTS_DIR" "$NAMING_SOURCE_KIND" "$NAMING_SOURCE_FILE" "$NAMING_SOURCE_REASON" "$NAMING_POLICY_JSON"
        else
