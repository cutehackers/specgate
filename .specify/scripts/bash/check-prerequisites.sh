...
eval $(get_feature_paths "$FEATURE_DIR_ARG") || exit 1
    ...
    if $PATHS_ONLY; then
        if $JSON_MODE; then
            printf '{"REPO_ROOT":"%s","FEATURE_DIR":"%s","FEATURE_DOCS_DIR":"%s","FEATURE_SPEC":"%s","CODE_DOC":"%s","RESEARCH":"%s","DATA_MODEL":"%s","QUICKSTART":"%s","SCREEN_ABSTRACTION":"%s","CONTRACTS_DIR":"%s","NAMING_SOURCE_KIND":"%s","NAMING_SOURCE_FILE":"%s","NAMING_SOURCE_REASON":"%s","NAMING_POLICY_JSON":%s,"LAYER_RULES_SOURCE_KIND":"%s","LAYER_RULES_SOURCE_FILE":"%s","LAYER_RULES_SOURCE_REASON":"%s","LAYER_RULES_RESOLVED_PATH":"%s","LAYER_RULES_HAS_LAYER_RULES":"%s","LAYER_RULES_PARSE_BLOCKED_BY_PARSER":"%s","LAYER_RULES_POLICY_JSON":%s,"LAYER_RULES_PARSE_EVENTS":%s,"LAYER_RULES_PARSE_SUMMARY":%s}\n' \
            ...
            "$NAMING_SOURCE_KIND" "$NAMING_SOURCE_FILE" "$NAMING_SOURCE_REASON" "$NAMING_POLICY_JSON" "$LAYER_RULES_SOURCE_KIND" \
            "$LAYER_RULES_SOURCE_FILE" "$LAYER_RULES_SOURCE_REASON" "$LAYER_RULES_RESOLVED_PATH" "$LAYER_RULES_HAS_LAYER_RULES" "$LAYER_RULES_PARSE_BLOCKED_BY_PARSER" \
