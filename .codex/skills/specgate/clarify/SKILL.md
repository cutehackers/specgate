...
`research.md` is not a primary `/clarify` artifact; it is maintained in `/specify` and may be refined here when ambiguity decisions change.
...
2. Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only --feature-dir "<abs path>"` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If `CLARIFY_DOC` does not exist, create it from `.specify/templates/clarify-template.md` if available; otherwise create an empty file.

3. Load the current spec file. Perform a structured ambiguity & coverage scan using this taxonomy. For each category, mark status: Clear / Partial / Missing. Produce an internal coverage map used for prioritization (do not output raw map unless no questions will be asked).

   UI abstraction policy (always on):
   - Clarification must stay at behavior/contract level (`input`, `event`, `ui_state`, `output`, `error_state`).
   - Never ask for or encode concrete UI details (layout/style/animation/component composition).
   - If concrete UI wording already exists in the spec, normalize it to behavior outcomes during integration.
   ...
   - Naming policy source and entity/event naming consistency between spec, data model, and screen contracts
...
4. Generate (internally) a prioritized queue of candidate clarification questions (maximum 5). Do NOT output them all at once. Apply these constraints:
    ...
    - Exclude all concrete UI preference questions (colors, typography, spacing, visual polish, component-level layout).
...
6. Integration after EACH accepted answer (incremental update approach):
    ...
    - Keep `CLARIFY_DOC` as a temporary session log synchronized with `spec.md#Clarifications`.
    - For the first integrated answer in this session:
       - Ensure a `## Clarifications` section exists (create it just after the highest-level contextual/overview section per the spec template if missing).
       - Under it, create (if not present) a `### Session YYYY-MM-DD` subheading for today.
       - Ensure matching `### Session YYYY-MM-DD` heading exists in `CLARIFY_DOC`.
    - Append a bullet line immediately after acceptance: `- Q: <question> → A: <final answer>`.
    - Append the same bullet line in `CLARIFY_DOC` session log.
    - Then immediately apply the clarification to the most appropriate section(s):
       ...
       - Naming conflict → Normalize entity/event/screen identifiers to resolved naming policy across all touched sections and planning artifacts.
...
7. Required planning artifact generation (must be executed even when no new clarification questions were needed):
   ...
   - If artifacts already exist, update in place without losing completed task IDs and explicit test tags.
...
8. Validation (performed after EACH write plus final pass):
   ...
   - Naming policy consistency: touched sections align on naming source selected in step 1.1.
...
9. Report completion (after questioning loop ends or early termination):
   ...
   - Set pointer stage to `clarifying` and `current_doc` to `spec.md`.
