# SpecGate Commands (Codex)

This directory is the Codex command surface for `SpecGate`.
...
- Before marking `/codify` phase complete, run `.specify/scripts/bash/check-implementation-quality.sh`.
- Before finalizing test execution, run `.specify/scripts/bash/check-test-coverage-targets.sh`.
