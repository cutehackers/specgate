...
1.1 **Resolve naming policy source**:
   ...
   - Resolve naming policy with this precedence:
     1. Use `<FEATURE_DIR>/docs/ARCHITECTURE.md` or `<FEATURE_DIR>/docs/architecture.md` when at least one of the following is true:
        - `Naming Rules`
        - `Naming Convention`
        - `Naming Policy`
        - Heading regex match: `^#{1,4}\s*Naming\s+(Rules|Convention|Policy)\s*$` (case-insensitive)
        - The section contains a non-empty JSON code block naming convention
     ...
     3. If still unavailable, apply repository default naming guardrails (non-blocking in default flow; strict production workflows require pre-correction).
   - Persist `NAMING_SOURCE_FILE` for final reporting.
