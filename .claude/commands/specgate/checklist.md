...
1. **Setup**:
   ...
   - Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only --feature-dir "<abs path>"` from repo root and parse JSON for FEATURE_DIR and FEATURE_DOCS_DIR.
     ...
     - `AVAILABLE_DOCS` can be used as optional context only.
...
5. **Generate checklist** - Create "Unit Tests for Requirements":
   - Create `FEATURE_DOCS_DIR/checklists/` directory if it doesn't exist
...
When reporting, emit at minimum:

- `CREATED_FILE`: absolute path of checklist file
- `ITEM_COUNT`: number of created `[ ] CHK###` rows
- `SECTION_COUNT`: number of checklist sections created
- `DOMAIN`: domain tag used for filename (e.g., `ux`, `api`)
- `FEATURE_DOC_DIR`: absolute path to feature docs
- `NEXT_ACTION`: which command should run next and why

If no checklist rows are generated, fail with an explicit reason and do not create a file.
