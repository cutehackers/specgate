...
1.1 **Resolve naming policy source**:
   ...
   - Determine `NAMING_SOURCE_FILE` with this precedence:
     1. Use `<FEATURE_DIR>/docs/ARCHITECTURE.md` or `<FEATURE_DIR>/docs/architecture.md` when all of the following are satisfied:
        - Section heading is one of `Naming Rules`, `Naming Convention`, `Naming Policy`
        - Heading matches regex `^#{1,4}\s*Naming\s+(Rules|Convention|Policy)\s*$` (case-insensitive)
        - The section contains at least one line of machine-readable naming content
     2. If the above does not match, fallback:
        - `<FEATURE_DIR>/docs/constitution.md`
        - `<FEATURE_DIR>/constitution.md`
        - `<REPO_ROOT>/.specify/memory/constitution.md`
     3. If all fail, apply repository default naming guardrails (do not block on missing convention for naming policy).
   - Record selected source (`ARCHITECTURE`, `CONSTITUTION`, `DEFAULT`) for subsequent steps.
...
5. Write or update SPEC_FILE in an idempotent way:
   ...
   - Ensure spec metadata includes `Naming Source` and records the `NAMING_SOURCE_FILE` origin.
   - If `ARCHITECTURE` source is unavailable, add a short fallback reason in `Architecture Compliance`.
